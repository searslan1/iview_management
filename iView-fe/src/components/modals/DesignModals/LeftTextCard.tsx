...
import { useLeftTextStore } from '../../../store/DesignModals/useLeftTextStore';
...
interface LeftTextCardProps {
    componentList: ComponentProps[];
    onComponentSelect: (selectedComponent: string) => void;
}

const LeftTextCard: React.FC<LeftTextCardProps> = ({ componentList, onComponentSelect }) => {
    // Zustand store selectors for LeftTextCard state
    const {
        componentName,
        mediaType,
        description,
    ...
    const handleSave = () => {
        saveLeftTextCardState(); // Calls Zustand's save logic
